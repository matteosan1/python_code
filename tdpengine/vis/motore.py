...
class Turno:
  def __init__(self):
F(turn) = wl/l + weDe + wcDc
l velocita
De distanza dalla fine
Dc distanza dal "centro"
(vale il minor valore)


class Heuristic:
  def __init__(self, point):
    wt = 1 # pese time-space cost
    Ct = 
F(heuristic) = wtCt + wrCr + wmCm + weDe
Ct time-space cost, prevent collisions (solves dynamic obstacles problem), 0 at the next position, otherwise 100
Cr rotational cost, orientation difference from previous direction
Cm spatial cost, defined by the map
